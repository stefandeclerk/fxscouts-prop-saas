  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <Card>
        ...
        <div className="border-b border-line px-5 py-3 text-[13px] text-ink/80">After every sync the gateway hashes the whole ledger, chains it to the previous seal and signs it. A ledger handed over later that does not hash to a sealed value has been altered since. Once a day every new seal is also anchored in the Bitcoin blockchain through OpenTimestamps, so the date of the record cannot be moved by anyone, FxScouts included. Anyone can check a record at <Link href="/verify" className="text-accent">/verify</Link>.</div>
        {b.seals.length === 0 ? <div className="px-5 py-8 text-center text-muted">No seal yet; the first is made after the first successful sync.</div> : (
          <div className="overflow-x-auto"><table className="w-full border-collapse">
            <thead><tr><th className="th">Seal</th><th className="th">Sealed at</th><th className="th num hidden md:table-cell">Deals</th><th className="th hidden lg:table-cell">Ledger hash</th><th className="th hidden xl:table-cell">Chained to</th><th className="th">Signed</th><th className="th">Anchored</th></tr></thead>
            <tbody>{b.seals.map((s) => (
              <tr key={s.seq}><td className="td font-semibold">#{s.seq}</td><td className="td whitespace-nowrap">{shortDateTime(s.sealed_at)} UTC</td><td className="td num hidden md:table-cell">{s.deals_count.toLocaleString("en-GB")}</td><td className="td mono text-xs hidden lg:table-cell">{s.ledger_hash.slice(0, 20)}…</td><td className="td mono text-xs hidden xl:table-cell">{s.prev_hash ? `${s.prev_hash.slice(0, 12)}…` : "first"}</td><td className="td">{s.signature ? <Status tone="ok">Yes</Status> : <Status tone="pend">No</Status>}</td>
                <td className="td">{s.anchor === undefined ? <span className="text-muted">–</span> : !s.anchor ? <Status tone="pend">Tonight</Status> : s.anchor.status === "confirmed" ? <><Status tone="ok">Bitcoin</Status><span className="block text-xs text-muted">block {s.anchor.block_height?.toLocaleString("en-GB")}</span></> : <Status tone="warn">Awaiting block</Status>}</td></tr>
            ))}</tbody>
          </table></div>
        )}
      </Card>
