...
export default function AdminShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 grid-rows-[auto_1fr] lg:grid-cols-[232px_1fr] lg:grid-rows-1">
      <aside className="relative isolate flex flex-row flex-wrap items-center gap-1 overflow-hidden border-b border-line bg-bg px-3.5 py-3 text-ink lg:sticky lg:top-0 lg:h-screen lg:flex-col lg:items-stretch lg:border-b-0 lg:border-r lg:py-5">
        <SidebarWaves />
        <Link href="/admin" className="mr-2 flex flex-col gap-1 px-2.5 lg:mr-0 lg:pb-4" aria-label={`${PRODUCT_FULL} admin`}>
          ...
          <span className="hidden text-[11px] font-medium tracking-wide text-muted lg:block">{PRODUCT} · Admin</span>
        </Link>
        {NAV.map((n) => <NavLink key={n.href} item={n} active={n.href === "/admin" ? pathname === "/admin" : pathname.startsWith(n.href)} />)}
        <div className="relative ml-auto text-[12px] text-ink/70 lg:-mx-1 lg:ml-0 lg:mt-auto lg:rounded-lg lg:bg-white/75 lg:px-2 lg:py-2 lg:backdrop-blur">
          <Link href="/app" className="hover:text-ink">Firm console</Link>
        </div>
      </aside>
