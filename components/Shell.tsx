...
export function NavLink({ item, active }: { item: NavItem; active: boolean }) {
  ...
  return (
    <Link href={item.href} className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 font-medium transition-colors ${active ? "bg-ink-soft text-white" : "text-ink/75 hover:bg-ink/6 hover:text-ink"}`}>
      <Icon className={`h-[18px] w-[18px] ${active ? "text-[#0fe0e0]" : ""}`} strokeWidth={1.75} />
      {item.label}
    </Link>
  );
}
...
export default function Shell({ children, firmName, role, staff = false }: { children: React.ReactNode; firmName: string; role: string; staff?: boolean }) {
  ...
  return (
    <div className="grid min-h-screen grid-cols-1 grid-rows-[auto_1fr] lg:grid-cols-[232px_1fr] lg:grid-rows-1">
      <aside className="relative isolate flex flex-row flex-wrap items-center gap-1 overflow-hidden border-b border-line bg-white px-3.5 py-3 text-ink lg:sticky lg:top-0 lg:h-screen lg:flex-col lg:items-stretch lg:border-b-0 lg:border-r lg:py-5">
        <SidebarWaves />
        <Link href="/app" className="mr-2 flex flex-col gap-1 px-2.5 lg:mr-0 lg:pb-4" aria-label={PRODUCT_FULL}>
          {/* eslint-disable-next-line @next/next/no-img-element */}
          <img src="/fxscouts-logo-dark.svg" alt="FxScouts" className="h-[22px] w-auto" />
          <span className="hidden text-[11px] font-medium tracking-wide text-muted lg:block">{PRODUCT} · Console</span>
        </Link>
        {MAIN.map((i) => <NavLink key={i.href} item={i} active={active(i.href)} />)}
        <div className="hidden px-2.5 pt-4 pb-1.5 text-[11px] font-semibold uppercase tracking-wider text-muted lg:block">Firm</div>
        {OPS.map((i) => <NavLink key={i.href} item={i} active={active(i.href)} />)}
        <div className="relative -mx-1 mt-auto hidden items-center gap-2.5 rounded-lg bg-white/75 px-1 py-2 backdrop-blur lg:flex">
          <span className="grid h-[30px] w-[30px] place-items-center rounded-full bg-ink-soft text-xs font-semibold text-white">{firmName.slice(0, 2).toUpperCase()}</span>
          <span className="min-w-0">
            <span className="block truncate">{firmName}</span>
            <small className="block text-xs text-muted">{role.charAt(0).toUpperCase() + role.slice(1)}</small>
          </span>
      ...
      <div className="min-w-0">
        {staff && (
          <div className="flex items-center justify-between bg-warn px-4 py-1.5 text-[12.5px] font-medium text-white md:px-6">
            <span>You are viewing <b>{firmName}</b>'s console as an admin. Anything you change here is changed for them.</span>
