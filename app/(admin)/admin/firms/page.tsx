          ...
          <tbody>
            {firms.map((f) => (
              <tr key={f.id} className="hover:bg-bg">
                <td className="td"><Link href={`/admin/firms/${f.id}`} className="font-semibold hover:text-accent">{f.name}</Link><span className="block text-xs text-muted">Since {shortDateTime(f.createdAt)}{f.customerId ? ` · customer ${f.customerId.slice(0, 8)}…` : ""}</span></td>
                ...
                <td className="td">{ago(f.lastEventAt, now)}{f.unseenEvents ? <span className="block text-xs text-muted">{f.unseenEvents} unseen</span> : null}</td>
                <td className="td num"><a href={`/api/admin/act-as?firm=${f.id}`} className="btn btn-sm">Open as staff</a></td>
              </tr>
            ))}
